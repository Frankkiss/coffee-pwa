...
- 粘贴文字、OCR 已有文字及已批准的图片/多模态解析必须联网并通过 Supabase Edge Function；不接受 URL 抓取；
...
若文件已经触发下载但元数据记录失败，界面必须明确区分为“文件已下载、提醒记录未保存”，不得误报下载失败或诱导用户重复下载。
...
5. 要求用户输入明确确认文本。
6. 客户端为本次确认生成稳定的 `restoreRequestId`，通过所有同步与恢复 RPC 共用的用户级事务锁，暂停该用户其他同步写入并执行 `restore_backup_v2`。
...
客户端的文件解析、预览、导出和恢复必须绑定当前用户与单调递增的操作代次；换文件、取消、换用户或组件卸载后，旧异步结果不得回写界面。全量回滚资格仅对同一次文件选择、最新的全量预览和已成功触发的恢复前下载有效，任一条件变化都立即失效。恢复操作必须单飞；一次确认只生成一个 `restoreRequestId`，响应丢失时只能以同一 ID 重试。

全量回滚 RPC 返回后，客户端必须强制执行一次同步拉取，并等待本地运行时确认已采用响应中的新 `syncEpoch`，才能显示“恢复完成”。

不物理删除当前用户记录。v1 备份因范围不完整，只能迁移后执行安全合并，不能用于全量回滚。

全量回滚必须使用短期幂等回执处理“服务器已提交但客户端未收到响应”的情况。回执以
`(user_id, restore_request_id)` 唯一标识，并绑定模式与备份 checksum；回执与业务恢复、`syncEpoch` 递增在同一事务内提交。
相同 `restoreRequestId`、相同模式和 checksum 的重试直接返回首次保存的完整响应，不再次写业务表、不再次递增 `syncEpoch`；
相同 `restoreRequestId` 携带不同模式或 checksum 必须拒绝。用户以后明确再次恢复同一备份时生成新的 `restoreRequestId`，视为一次新的回滚。
客户端只能在一次确认流程内复用该 ID，不能在失败后静默换 ID 自动重试。回执属于短期基础设施去重数据，不是用户可见修改历史；
清理窗口不得短于客户端最大自动重试窗口。
...
- `restore_backup_v2`：根据明确模式执行事务式安全合并或全量回滚；全量回滚另需稳定的 `restoreRequestId`。
...
AI 推荐和来源解析继续由现有 Supabase Edge Functions 承担。除密钥仅存在服务器环境变量外，来源解析仅接受粘贴文字、OCR 已有文字和已批准的图片/多模态入口；不得读取用户提交的 URL，也不得发起页面抓取。函数必须限制单用户调用频率、入站请求字节数与送入模型的文字长度，且不把完整敏感原文写入日志或长期备份。伪造的 `url` 字段必须稳定返回 400 且零外部请求。
