...
- 粘贴文字和用户主动上传的包装图片解析必须联网并通过 Supabase Edge Function；不接受 URL 抓取；
...
AI 推荐和来源解析继续由现有 Supabase Edge Functions 承担。除密钥仅存在服务器环境变量外，来源解析仅接受粘贴文字和用户主动上传的单张包装图片；不得读取用户提交的 URL，也不得发起页面抓取。函数必须限制单用户调用频率、入站请求字节数、图片解码字节数与送入模型的文字长度，且不把完整敏感原文或图片写入日志或长期备份。伪造的 `url` 字段必须稳定返回 400 且零外部请求。
