...
自 2026-08-26 起，大白话冲煮反馈、历史配方完整度、缺失比例处理及四类 AI 冲煮步骤安全校验以
[`2026-08-26-recommendation-feedback-and-step-safety-design.md`](./2026-08-26-recommendation-feedback-and-step-safety-design.md)
为详细设计依据。
...
不兼容时自动改为该方式的默认方法：热手冲和冰手冲默认手冲，冷萃默认冷萃，意式默认意式。冷萃类型已经完整表达冲煮方式，
因此选择冷萃时隐藏“具体方法”，内部固定保存兼容值“冷萃”；其他规范化方式继续显示原有具体方法下拉框。用户选择会立即
触发联动，不允许保存规范化方式与具体方法相互矛盾的新记录。

冰量输入按方式渐进显示：冰手冲始终显示；冷萃仅在子类型为“浓缩基底”时显示；热手冲、冷萃直接饮用和意式隐藏并清空
不适用的冰量。冰手冲的粉水比只按热水量除以粉量计算，冰量单独记录、展示和校验，不得计入粉水比；界面在冰手冲语境中
标注“粉水比（仅热水）”。冷萃浓缩基底的冰量表示饮用时加入的冰块，同样不得计入冷萃原液的萃取粉水比。
