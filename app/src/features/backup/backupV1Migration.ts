...
export async function normalizeV1ForSafeMerge(
  ...
  const data = {
    ...
    brewLogs: source.data.brewLogs.map((brewLog) => ({
      ...brewLog,
      brew_mode: brewLog.brew_mode ?? null,
      brew_variant: brewLog.brew_variant ?? null,
      ice_grams: brewLog.ice_grams ?? null,
      beverage_grams: brewLog.beverage_grams ?? null,
    })),
    brewTemplates,
    aiRecommendations: [],
    sourceImports: [],
